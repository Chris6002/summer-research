...
if __name__ == '__main__':
    ...
    input = torch.from_numpy(data[3:, :-1]).cuda()
    target = torch.from_numpy(data[3:, 1:]).cuda()
    test_input = torch.from_numpy(data[:3, :-1]).cuda()
    test_target = torch.from_numpy(data[:3, 1:]).cuda()
    # build the model
    seq = Sequence()
    seq.double().cuda()

...
class1_path = 'parameter/0_4.pth.tar'
class2_path = 'parameter/0_4.pth.tar'
class3_path = 'parameter/0_4.pth.tar'
class4_path = 'parameter/0_4.pth.tar'
meta_path =   'parameter/0_4.pth.tar'
path_following = Monitor(class1_path)
people_following = Monitor(class2_path)
confronting = Monitor(class3_path)
crossing = Monitor(class4_path)
meta = Monitor(meta_path)
...
# Dagger command file:
if args.Dagger == 1:
    f = open(Command_file + str(args.classnum)+'_Dagger_' +
             str(args.iter)+'_command'+'.csv', 'w')
    fnames = ['name', 'frame', 'steering',
              'speed', 'category', 'stage', 'useful']
...
# verify time file:
if args.verify == 1:
    time_file = open(Command_file + str(args.classnum) +
                     '_Time_'+str(args.iter)+'.csv', 'w')
...
try:
    while True:
        ...
        try:
            ...
            if abs(dis4) > 500:
                ...
                if adjust_flag:
                    ...
                    if args.verify == 1:
                        ...
                        print('Manual Now Dagger auto ends: ',
            ...
            if bool_retrieve:
                if adjust_flag == 0:
                    classnum = meta.inference(frame).item()
            ...
            if situation == 1 and ch3 > 1900:
                ...
                if args.verify == 1:
                    time_file.close()
                    time_file = open(
                        Command_file + str(args.classnum)+'_Time_'+str(args.iter)+'.csv', 'a')
                    time_fnames = ['end_frame', 'time']
                    time_writer = csv.DictWriter(
                        time_file, fieldnames=time_fnames)
            pre_save = save
